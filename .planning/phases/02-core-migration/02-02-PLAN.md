...
must_haves:
  truths:
    ...
    - "Centerline muscles (abs, head, hair, neck) have single entries"
...
**For centerline muscles** (have path.common, may have path.left/right for visual symmetry):
- Keep as ONE entry
- Combine all paths: `{ slug: "{muscle}", pathData: [...(path.common || []), ...(path.left || []), ...(path.right || [])] }`

Bilateral muscles to split (in bodyFront.ts):
- chest -> left-chest, right-chest (has path.left/path.right in current file - see lines 8-16)
...
Count entries: should have ~34 entries (15 bilateral pairs + 4 centerline in front view).
